...
import moment from 'moment'; // moment.js для удобного форматирования дат
...
const AmenitiesBuyPage: React.FC = () => {
    ...
    const [deletionMessage, setDeletionMessage] = useState<string>(''); // Сообщение об удалении
        ...
        axios.get('http://127.0.0.1:8000/api/tickets/search', {
            ...
            .then(response => {
                ...
                if (fetchedTickets.length > 0) {
                    ...
                    fetchPurchasedAmenities(fetchedTickets[0].id); // Получение купленных удобств
                }
            })
            .catch(error => {
    ...
    const fetchPurchasedAmenities = async (ticketId: number) => {
        try {
            const response = await axios.get(`http://127.0.0.1:8000/api/amenitiestickets/`, {
                params: { ticket: ticketId } // Фильтрация по id билета
            });
            ...
            purchasedAmenities.forEach((item: { amenity: number }) => {
                updatedSelectedAmenities[item.amenity] = true; // Отметить как выбранный
            });
    ...
    const handleRemoveAmenity = async (amenityId: number) => {
        ...
        try {
            ...
            if (ticketAmenityId) {
                await axios.delete(`http://127.0.0.1:8000/api/amenitiestickets/${ticketAmenityId}/`); // Удалить запрос

                // Обновление локального состояния для удаления удаленного удобства
                setSelectedAmenities((prevState) => {
                    const newState = { ...prevState };
                    delete newState[amenityId]; // Удалить выбранное удобство
                    return newState;
                });

                setDeletionMessage(`Amenity has been removed successfully.`); // Сообщение о подтверждении удаления
            }
        } catch (error) {
    ...
    const fetchTicketAmenityId = async (ticketId: number, amenityId: number): Promise<number | null> => {
        try {
            ...
            return amenityTicket ? amenityTicket.id : null; // Возвращает ID удобства билета
        } catch (error) {
    ...
    const handleSaveAndConfirm = () => {
        ...
        selectedAmenitiesList.forEach(async (amenity) => {
            const price = parseFloat(amenity.price);
            const isFreeForCabin = selectedTicket && amenity.cabin_types.includes(selectedTicket.cabintypeid);
            const finalPrice = isFreeForCabin ? 0 : price + (price * 0.05); // Цена с налогом, если применимо
    ...
    const isFlightSelectable = (flightDate: string, flightTime: string): boolean => {
        const flightDateTime = moment(`${flightDate} ${flightTime}`, 'YYYY-MM-DD HH:mm'); // Объединяем дату и время рейса
        const currentDateTime = moment(); // Текущая дата и время

        const timeDifference = flightDateTime.diff(currentDateTime, 'hours'); // Разница в часах
        return timeDifference > 24; // Если до рейса больше 24 часов, его можно выбрать
    };
    ...
    return (
        <div>
            ...
            {deletionMessage && <p style={{ color: 'green' }}>{deletionMessage}</p>} {/* Отображение сообщения об удалении */}
                    ...
                    <select onChange={(e) => setSelectedSchedule(Number(e.target.value))} value={selectedSchedule || ''}>
                        ...
                        {schedules.map(schedule => (
                            <option 
                                key={schedule.id} 
                                value={schedule.id}
                                disabled={!isFlightSelectable(schedule.date, schedule.time)} // Отключаем, если рейс нельзя выбрать
                            >
